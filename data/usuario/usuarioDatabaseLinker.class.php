<?php
include_once trim(__DIR__, 'usuario').'/conexion/conectionData.php';
include_once trim(__DIR__, 'usuario').'/conexion/dataBaseConnector.php';
//include_once 'data/conexion/conectionData.php';
//include_once 'data/conexion/dataBaseConnector.php';
...
class UsuarioDatabaseLinker
{
	...
	{
		$this->dbusuario = new dataBaseConnector(HOST,0,DBvieja,USRDBvieja,PASSDBvieja);
	}
	...
	function agregarUsuario($detalle, $nombre, $contrasena, $entidad)
	{
		...
		$query="SELECT 
					IFNULL(MAX(iduser_usuario),0)+1 AS proximo
				FROM 
				  	user_usuario;";
		...
		$query = "INSERT INTO user_usuario (iduser_usuario, detalle, nombre, contrasena, fecha, entidad) VALUES (".$nroIdUsuario.",'".strtolower($detalle)."','".strtoupper($nombre)."', '".$contrasenamd5."', now(), '".$entidad."');";
	...
	{
		$query="SELECT detalle FROM user_usuario WHERE entidad = '".$entidad."' AND detalle = '".strtolower($usuario)."' AND habilitado = 1;";
	...
	{	
		$query="INSERT INTO user_usuario_permiso (iduser_usuario, iduser_permiso) VALUES (".$idusuario.", '".$perfil."');";
	...
	function eliminarUsuario($data)
	{
		...
		$query="UPDATE user_usuario SET habilitado = 0 WHERE entidad = '".$data['entidad']."' AND detalle = '".strtolower($data['usuario'])."' and habilitado = 1;";
    ...
    function confirmarPermisosUsuario($data)
	{
        ...
        try
        {
            ...
            $query = "DELETE FROM user_usuario_permiso WHERE iduser_usuario = " . $idusuario;
            $this->dbusuario->ejecutarAccion($query);
            
            // AGREGAR LOS NUEVOS PERMISOS
            for($i = 0; $i < count($permisos); $i++)
            {
                $query = "INSERT INTO user_usuario_permiso (iduser_usuario, iduser_permiso) VALUES (".$idusuario.", '".$permisos[$i]."');";
                $this->dbusuario->ejecutarAccion($query);
			}
    ...
    function getUsuariosJson($entidad, $page, $rows)
    {
        ...
        { 
            ...
            //id de fila
            $response->rows[$i]['id'] = $usuario['iduser_usuario']; 
    ...
    function cambiarContrasenaUsuario($usuariodetalle, $data)
    {
	    ...
	    try
	    {
            ...
            $query = "SELECT contrasena FROM user_usuario WHERE detalle = '".strtolower($usuariodetalle)."' AND entidad ='".$data['entidad']."'";
            $this->dbusuario->ejecutarQuery($query);
            ...
            {
                // CAMBIAR CONTRASEÑA
                $query = "UPDATE user_usuario SET contrasena = '".$contrasenaNuevaMd5."' WHERE detalle = '".strtolower($usuariodetalle)."' AND entidad ='".$data['entidad']."'";
                $this->dbusuario->ejecutarAccion($query);
	...
	function cambiarNombreUsuario($usuariodetalle, $data)
    {
        ...
        try
        {
            ...
            // CAMBIAR NOMBRE
            $query = "UPDATE user_usuario SET nombre = '".$nombre."' WHERE detalle = '".strtolower($usuariodetalle)."' AND entidad = '".$data['entidad']."'";
            $this->dbusuario->ejecutarAccion($query);
	...
	function traerPermisosDelUsuario($idusuario, $entidad)
    {
        $query = "SELECT 
        				p.iduser_permiso, 
        				p.detalle, 
        				up.iduser_usuario IS NOT NULL as tiene
                	FROM 
                		user_permiso p LEFT JOIN 
                		user_usuario_permiso up ON up.iduser_permiso = p.iduser_permiso AND up.iduser_usuario = ".$idusuario ."
                	WHERE
		...
		for($i = 0 ; $i < $this->dbusuario->querySize; $i++)
		{
			...
			$arrdos = array(
	                            'idpermiso' => $result['iduser_permiso'],
	...
	function permisosDeUsuario($idusuario)
    {
        $query = "SELECT 
        				iduser_permiso
                	FROM
                		user_usuario_permiso
                	WHERE
                		iduser_usuario = ".$idusuario .";";
	...
	function traerPermisos($entidad)
	{
		...
		{
			$result = $this->dbusuario->fetchRow($query);
			$arrdos = array('idpermiso' => $result['iduser_permiso'],'detalle' => $result['detalle']);
			$arr[] = $arrdos;
		}
